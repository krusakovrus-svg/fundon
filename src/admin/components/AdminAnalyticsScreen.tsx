...
function getToneClasses(tone: 'positive' | 'negative' | 'neutral') {
  switch (tone) {
    case 'positive':
      return 'bg-emerald-50/80 text-emerald-700 ring-1 ring-emerald-100';
    case 'negative':
      return 'bg-rose-50/85 text-rose-700 ring-1 ring-rose-100';
    case 'neutral':
      return 'bg-slate-100 text-slate-600 ring-1 ring-slate-200';
  }
}

function getKpiTrendLabel(tone: 'positive' | 'negative' | 'neutral') {
  switch (tone) {
    case 'positive':
      return 'Рост';
    case 'negative':
...
function formatThousandsOfRubles(value: number) {
  if (value === 0) {
    return '0 ₽';
  }
...
function getAverageDonationLabel() {
  const average = Math.round(adminAnalyticsDonationAmounts.reduce((sum, value) => sum + value, 0) / adminAnalyticsDonationAmounts.length);
  return formatThousandsOfRubles(average);
}
...
function KpiSparkline({ id, points, accent }: { id: string; points: number[]; accent: string }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="h-11 w-full" aria-hidden="true">
      ...
      <polyline fill="none" stroke={accent} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" points={toPolyline(seriesPoints)} />
    </svg>
  );
}
...
function KpiCard({
  ...
  return (
    <div className="flex h-full min-h-[10.75rem] flex-col rounded-[22px] border border-black/[0.05] bg-white/92 px-5 py-4 shadow-[0_16px_34px_rgba(15,23,42,0.05)]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[0.78rem] font-semibold uppercase tracking-[0.16em] text-slate-400">{label}</p>
          <p className="mt-2 text-[1.88rem] font-semibold tracking-tight tabular-nums text-slate-900">{value}</p>
        </div>
        <span className={cn('inline-flex rounded-full px-2.5 py-1 text-[0.68rem] font-semibold', getToneClasses(trendTone))}>{getKpiTrendLabel(trendTone)}</span>
      </div>
      <p className="mt-2 text-[0.78rem] leading-5 text-slate-400">{trend}</p>
      <div className="mt-auto border-t border-black/[0.045] pt-3">
    ...
    <div className="space-y-4">
      <ChartLegend
        items={[
          ...
          { label: 'Активная аудитория', color: palette.green },
      ...
      <div className="rounded-[20px] border border-black/[0.045] bg-[linear-gradient(180deg,#fcfdff_0%,#f8fafd_100%)] px-4 py-4">
        <svg viewBox={`0 0 ${width} ${height}`} className="h-[18rem] w-full" aria-hidden="true">
          ...
          <path d={toLinePath(baselinePoints)} fill="none" stroke={palette.slate} strokeOpacity="0.34" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
          ...
          {adminAnalyticsGrowthLabels.map((label, index) => {
            ...
            return (
              <g key={label}>
                ...
                <rect x={barX} y={newUsersY} width={barWidth} height={newUsersHeight} rx="8" fill={palette.blue} fillOpacity="0.9" />
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <ChartLegend
          items={[
            { label: 'Объём поддержки', color: palette.blue },
            { label: 'Ошибки оплаты', color: palette.coral },
            { label: 'Базовый ритм', color: palette.graphite, muted: true }
          ]}
        />
        <span className="rounded-full bg-[#f5f8fd] px-3 py-1.5 text-[0.76rem] font-semibold text-slate-500">Средний слот: {getAverageDonationLabel()}</span>
      </div>
      ...
      <div className="rounded-[20px] border border-black/[0.045] bg-[linear-gradient(180deg,#fcfdff_0%,#f8fafd_100%)] px-4 py-4">
        <svg viewBox={`0 0 ${width} ${height}`} className="h-[18rem] w-full" aria-hidden="true">
          {[0, 0.33, 0.66, 1].map((step) => {
            const y = top + (baseline - top) * (1 - step);
            const axisValue = formatThousandsOfRubles(Math.round(maxBarValue * step));
          ...
          <path d={toLinePath(averagePoints)} fill="none" stroke={palette.graphite} strokeOpacity="0.24" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
          <path d={toLinePath(errorPoints)} fill="none" stroke={palette.coral} strokeOpacity="0.82" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          ...
          {adminAnalyticsDonationLabels.map((label, index) => {
            ...
            return (
              <g key={label}>
                ...
                <rect x={barX} y={y + barHeight * 0.18} width={barWidth} height={barHeight * 0.82} rx="8" fill={palette.blue} fillOpacity="0.88" />
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1.3fr)_0.88fr]">
      <div className="space-y-4 rounded-[20px] border border-black/[0.045] bg-[linear-gradient(180deg,#fcfdff_0%,#f8fafd_100%)] px-4 py-4">
        <ChartLegend
          items={[
            { label: 'Сессии в эфире', color: palette.blue },
            { label: 'Поддержка в эфире', color: palette.green },
            { label: 'Пиковая нагрузка', color: palette.graphite, muted: true }
          ]}
        />
        ...
        <svg viewBox={`0 0 ${width} ${height}`} className="h-[15rem] w-full" aria-hidden="true">
          ...
          <path d={toAreaPath(viewerPoints, baseline)} fill={palette.blueFill} fillOpacity="0.72" />
          <path d={toAreaPath(supportPoints, baseline)} fill={palette.greenFill} fillOpacity="0.66" />
          ...
          <path d={toLinePath(peakPoints)} fill="none" stroke={palette.graphite} strokeWidth="1.6" strokeOpacity="0.48" strokeLinecap="round" strokeLinejoin="round" />
      ...
      <div className="rounded-[20px] border border-black/[0.045] bg-[linear-gradient(180deg,#fcfdff_0%,#f8fafd_100%)] px-4 py-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[0.8rem] text-slate-500">Слоты с максимальной нагрузкой по эфиру</p>
          </div>
          ...
          {adminAnalyticsLiveMoments.map((moment) => (
            <div key={moment.id}>
              <div className="mb-2 flex items-center justify-between gap-3">
                ...
                <p className="text-[0.82rem] text-slate-500">{moment.value} тыс. / база {moment.baseline} тыс.</p>
              </div>
    ...
    <SectionCard title={title} subtitle={subtitle} className="h-full">
      <div className="space-y-2.5">
        {metrics.map((metric) => (
          <div
            key={metric.id}
            className="flex items-center justify-between gap-4 rounded-[18px] border border-black/[0.045] bg-[linear-gradient(180deg,#ffffff_0%,#fafbfe_100%)] px-4 py-3"
          >
            <div className="min-w-0">
              ...
              <p className="mt-1.5 text-[0.8rem] leading-5 text-slate-500">{metric.hint}</p>
            </div>
            <div className="shrink-0 text-right">
              <p className="text-[1.04rem] font-semibold tracking-tight tabular-nums text-slate-900">{metric.value}</p>
    ...
    <SectionCard title={title} subtitle={subtitle}>
      <div className="space-y-2.5">
        {rows.map((row) => (
          <div
            key={row.id}
            className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-4 rounded-[18px] border border-black/[0.045] bg-[linear-gradient(180deg,#ffffff_0%,#fafbfe_100%)] px-4 py-3"
          >
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 truncate text-[0.8rem] text-slate-500">{row.meta}</p>
            </div>
            <div className="text-right">
              <p className="text-[0.98rem] font-semibold tabular-nums text-slate-900">{row.value}</p>
    ...
    <SectionCard title="Сегменты пользователей" subtitle="Ключевые группы аудитории по объёму и качеству активности.">
      <div className="grid gap-2.5 sm:grid-cols-2 2xl:grid-cols-3">
        {adminAnalyticsSegments.map((segment) => (
          <div
            key={segment.id}
            className="rounded-[18px] border border-black/[0.045] bg-[linear-gradient(180deg,#ffffff_0%,#fafbfe_100%)] px-4 py-3.5"
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[0.88rem] font-semibold text-slate-800">{segment.label}</p>
                <p className="mt-1 text-[0.8rem] leading-5 text-slate-500">{segment.note}</p>
              </div>
              <span className="rounded-full bg-[#eef5ff] px-2.5 py-1 text-[0.72rem] font-semibold text-[#2f78d3]">{segment.delta}</span>
            </div>
            <p className="mt-3 text-[1.42rem] font-semibold tracking-tight tabular-nums text-slate-900">{segment.value}</p>
          </div>
        ))}
      </div>
    </SectionCard>
...
function AnomaliesCard() {
  return (
    <SectionCard title="Аномалии" subtitle="Короткие сигналы для ручной проверки и быстрого ответа.">
      <div className="space-y-2.5">
          ...
          >
            ...
            <div className="mt-3 space-y-2">
              <div className="rounded-[14px] bg-[#f8fafc] px-3 py-2.5">
                <p className="text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-slate-400">Что произошло</p>
                <p className="mt-1 text-[0.82rem] leading-5 text-slate-600">{item.description}</p>
              </div>
              <div className="rounded-[14px] bg-[#fffaf3] px-3 py-2.5">
                <p className="text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-amber-500">Почему важно</p>
                <p className="mt-1 text-[0.82rem] leading-5 text-slate-700">{item.impact}</p>
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    <section className="overflow-hidden rounded-[24px] border border-[#dbe7fb] bg-[linear-gradient(180deg,#ffffff_0%,#f6f9ff_100%)] shadow-[0_18px_40px_rgba(79,143,246,0.12)]">
      <div className="border-b border-[#e2ebfb] px-6 py-5">
        ...
        <p className="mt-1 text-sm text-slate-500">Короткая управленческая сводка по росту, донатам и возврату в продукт.</p>
      </div>
      ...
      <div className="space-y-3 p-6">
        {adminAnalyticsInsights.map((insight) => (
          <div
            key={insight.id}
            className="rounded-[18px] border border-[#e2ebfb] bg-white/90 px-4 py-3.5 shadow-[0_12px_24px_rgba(79,143,246,0.05)]"
          >
            <div className="flex items-center justify-between gap-3">
            ...
            <p className="mt-2 text-[0.82rem] leading-5 text-slate-600">{insight.note}</p>
          </div>
        ))}

        <div className="rounded-[18px] border border-[#d7e4fb] bg-[linear-gradient(180deg,#eef5ff_0%,#e8f1fe_100%)] px-4 py-3.5">
          <p className="text-[0.78rem] font-semibold uppercase tracking-[0.16em] text-[#2f78d3]">Фокус</p>
          <p className="mt-2 text-[0.88rem] leading-6 text-slate-700">{adminAnalyticsRecommendation}</p>
        </div>
      </div>
    </section>
  );
...
export function AdminAnalyticsScreen() {
  return (
    <div className="space-y-6">
      ...
      <section className="grid items-start gap-6 2xl:grid-cols-[minmax(0,1.55fr)_23rem]">
        <div className="space-y-6">
          ...
          <SectionCard title="Активность в эфире" subtitle="Поведение аудитории, поддержка и пиковые интервалы эфира.">
            <LiveActivityChart />
          </SectionCard>
          ...
          <div className="grid gap-6 xl:grid-cols-2 2xl:grid-cols-4">
            ...
            <MetricListCard title="Поведение поддержки" subtitle="Эфир, архив, быстрые суммы и своя сумма." metrics={adminAnalyticsSupportBehaviorMetrics} />
          </div>
        ...
        <div className="space-y-6">
          ...
          <RankingCard title="Лидеры после эфира" subtitle="Какие события лучше всего собирают поддержку после эфира." rows={adminAnalyticsPopularityEntities} />
          <InsightsCard />
        </div>
      </section>
    </div>
